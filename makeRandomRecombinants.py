...
def makeExamples(myS, myB, myC, myD, myF, myT, mym, myM, myR, mySep):
    ...
    open('recombination_'+str(myB)+'_'+str(myC)+'.msa.fa','w').write(myOutMSA)
    open('recombination_'+str(myB)+'_'+str(myC)+'.log','w').write(myOutLog)
    open('recombination_'+str(myB)+'_'+str(myC)+'.differences.txt','w').write(myOutDiff)
