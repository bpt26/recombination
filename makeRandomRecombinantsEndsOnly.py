...
def makeExamples(myS, myB, myC, myD, myF, myT, mym, myM, myR, mySep):
    ...
    while len(recSampleToSeq.keys()) < myS:
        ...
        myBPChoice = sorted(list(posToRef.keys()))[:5000]+sorted(list(posToRef.keys()))[-5000:]
        if myB == 1:
            bps = numpy.random.choice(myBPChoice, size=1, replace=False)
            bp1 = bps[0]
            ...
            bps = sorted(numpy.random.choice(myBPChoice, size=2, replace=False))
            ...
            bps = sorted(numpy.random.choice(myBPChoice, size=3, replace=False))
            ...
            bps = sorted(numpy.random.choice(myBPChoice, size=4, replace=False))
